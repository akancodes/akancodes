# 👋 Hello, I'm Bayram Akan

I’m an **Electronic and Computer Engineering student** at Dublin City University with a passion for technology, mobile app development, and embedded systems. I also run a mobile development agency, [AKES](https:akes.co), where we create innovative applications with a focus on UI/UX design and cross-platform functionality.

---

## 💼 About Me

- **Entrepreneur & Student:** Running a mobile development agency called **AKES** with my partner, [Omer](https://github.com/Skipperlla).
- **Experience:** React Native developer with back-end knowledge in Python.
- **Currently Learning:** Improving my skills in embedded systems, React Native, and various electronics tools.
- **Goal:** To gain expertise in business and marketing, alongside technical skills.

---

## 📚 Education

- **First-Year Electronic and Computer Engineering Student** at **Dublin City University**

---

## 🛠️ Technical Skills

- **Languages:** TypeScript, Python, C, C++, GoLang
- **Frameworks & Libraries:** React Native, Expo, NextJS
- **Tools & Platforms:** Ubuntu, Arch Linux, Wayland, KiCad, Fusion 360
- **Development Setup:** JetBrains font, LightDM, Arch Linux with eGPU support

---

## 🌐 Connect with Me

- **Email:** [bayram@akan.dev](mailto:bayram@akan.dev)
- **LinkedIn:** [akancodes](https://www.linkedin.com/in/akancodes/)

---

## 📊 GitHub Stats

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=akancodes&layout=compact&theme=dark)

---

## 🛠️ Tools & Technologies

![JavaScript](https://img.shields.io/badge/-JavaScript-072f5f?style=flat&logo=javascript)&nbsp;
![TypeScript](https://img.shields.io/badge/-TypeScript-072f5f?style=flat&logo=typescript)&nbsp;
![Python](https://img.shields.io/badge/-Python-072f5f?style=flat&logo=python)&nbsp;
![React Native](https://img.shields.io/badge/-ReactNative-072f5f?style=flat&logo=react)&nbsp;
![Expo](https://img.shields.io/badge/-Expo-072f5f?style=flat&logo=expo)&nbsp;
![Neovim](https://img.shields.io/badge/-Neovim-072f5f?style=flat&logo=neovim)&nbsp;
![Arch Linux](https://img.shields.io/badge/-ArchLinux-072f5f?style=flat&logo=arch-linux)&nbsp;
![React](https://img.shields.io/badge/-React-072f5f?style=flat&logo=react)&nbsp;
![Next.js](https://img.shields.io/badge/-Next.js-072f5f?style=flat&logo=next.js)&nbsp;
![Node.js](https://img.shields.io/badge/-Node.js-072f5f?style=flat&logo=node.js&logoColor=3C873A)&nbsp;
![Express](https://img.shields.io/badge/-Express-072f5f?style=flat&logo=express)&nbsp;
![MongoDB](https://img.shields.io/badge/-MongoDB-072f5f?style=flat&logo=mongodb)&nbsp;
![HTML](https://img.shields.io/badge/-HTML-072f5f?style=flat&logo=html5)&nbsp;
![CSS](https://img.shields.io/badge/-CSS-072f5f?style=flat&logo=css3&logoColor=2965f1)&nbsp;
![Bootstrap](https://img.shields.io/badge/-Bootstrap-072f5f?style=flat&logo=bootstrap)&nbsp;
![Tailwind CSS](https://img.shields.io/badge/-TailwindCSS-072f5f?style=flat&logo=tailwindcss)&nbsp;
![Docker](https://img.shields.io/badge/-Docker-072f5f?style=flat&logo=docker)&nbsp;
![Git](https://img.shields.io/badge/-Git-072f5f?style=flat&logo=git)&nbsp;
![Prisma](https://img.shields.io/badge/-Prisma-072f5f?style=flat&logo=prisma)

---

Thank you for visiting my profile! 😊
